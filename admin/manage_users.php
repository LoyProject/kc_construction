<?php
    ...
    $filter_role = isset($_GET['role']) && in_array($_GET['role'], ['Admin', 'Editor', 'View']) ? $_GET['role'] : 'all';
    $filter_status = isset($_GET['status']) && in_array($_GET['status'], ['Active', 'Inactive']) ? $_GET['status'] : 'all';
    $where_clauses = [];
    $params = [];

    if (!empty($search_term)) {
        $where_clauses[] = "(username LIKE :search_username OR email LIKE :search_email)";
    ...
    try {
        $count_sql = "SELECT COUNT(id) FROM users" . $where_sql;
        ...
        $sql = "SELECT id, username, email, role, created_at, status FROM users" . $where_sql . " ORDER BY id DESC LIMIT :limit OFFSET :offset";
        ...
        $users = [];
        echo "<div class='p-4 mb-4 text-sm text-red-700 bg-red-100 rounded-lg' role='alert'>Error fetching users. Please try again later.</div>";
    }

    $base_link_params = array_filter(['search' => $search_term, 'role' => $filter_role, 'status' => $filter_status]);
?>
...
<div class="flex justify-between items-center mb-6">
    ...
    <div class="flex space-x-4">
        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin'): ?>
            <a href="reset_user" class="inline-flex items-center justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-yellow-500 hover:bg-yellow-600 ml-2">
                <svg class="w-5 h-5 mr-2" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 11c1.657 0 3-1.343 3-3s-1.343-3-3-3-3 1.343-3 3 1.343 3 3 3zm0 2c-2.67 0-8 1.337-8 4v3h16v-3c0-2.663-5.33-4-8-4z" /></svg>
                Reset Password
            </a>
        <?php endif; ?>
        <a href="add_user" class="inline-flex items-center justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brand-blue hover:bg-blue-700">
...
<form action="manage_users" method="GET" class="grid grid-cols-1 md:grid-cols-5 gap-4 items-end mb-4">
    ...
    <div>
        <select name="role" id="filter_role" class="mt-1 block w-full px-3 py-2 bg-white border border-slate-300 rounded-md shadow-sm focus:outline-none focus:ring-brand-blue focus:border-brand-blue sm:text-sm">
            ...
            <option value="Editor" <?php if ($filter_role === 'Editor') echo 'selected'; ?>>Editor</option>
            <option value="View" <?php if ($filter_role === 'View') echo 'selected'; ?>>View</option>
        </select>
    </div>
    <div>
    ...
    <div class="flex items-center space-x-4">
        ...
        <a href="manage_users" class="text-red-500 text-base hover:underline">Clear Filters</a>
    </div>
</form>
...
<div class="overflow-x-auto bg-white rounded-lg shadow">
    <table class="min-w-full leading-normal">
        <thead class="bg-gray-200">
            <tr>
                <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Username</th>
                <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Email</th>
                <th class="px-5 py-3 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Role</th>
                ...
                <?php foreach ($users as $user): ?>
                    ...
                    <tr id="user-row-<?php echo $user['id']; ?>">
                        ...
                        <td class="px-5 py-3 border-b border-gray-200">
                            <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin') {
                                    $status_color = ($user['status'] === 'Active') ? 'bg-green-100 text-green-800' : 'bg-slate-200 text-slate-800';
                        ...
                        <td class="px-5 py-3 border-b border-gray-200">
                            <?php if (isset($_SESSION['role']) && $_SESSION['role'] !== 'View'): ?>
                                <a href="edit_user?id=<?php echo $user['id']; ?>" class="text-indigo-600 hover:text-indigo-900 mr-3">
                            ...
                            <?php if ($user['id'] !== $_SESSION['user_id']): ?>
                                <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'Admin'): ?>
                                    <button type="button" 
                                        class="text-red-600 hover:text-red-800 delete-user-btn" 
            ...
            <ul class="inline-flex items-center -space-x-px">
                <?php
                    ...
                    if ($current_page > 1) {
                        ...
                        echo '<li><a href="manage_users?' . http_build_query($prev_page_params) . '" class="px-3 py-2 ml-0 leading-tight text-slate-500 bg-white border border-slate-300 rounded-l-lg hover:bg-slate-100 hover:text-slate-700">Previous</a></li>';
                    }
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        if ($i == 1 || $i == $total_pages || ($i >= $current_page - $range && $i <= $current_page + $range)) {
                            ...
                            if ($i == $current_page) {
                                ...
                                echo '<li><a href="manage_users?' . http_build_query($page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 hover:bg-slate-100 hover:text-slate-700">' . $i . '</a></li>';
                            }
                        } else { if (!$show_ellipsis) { $show_ellipsis = true; } }
                    }
                    if ($current_page < $total_pages) {
                        $next_page_params = array_merge($pagination_params, ['page' => $current_page + 1]);
                        echo '<li><a href="manage_users?' . http_build_query($next_page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 rounded-r-lg hover:bg-slate-100 hover:text-slate-700">Next</a></li>';
                    }
                ?>
            </ul>
        </nav>
            ...
            tableBody.addEventListener('click', function(event) {
                ...
                if (statusButton) {
                    if (confirm('Are you sure you want to change the status of this user?')) {
                        ...
                        fetch('process_edit_user_status', {
                            method: 'POST',
