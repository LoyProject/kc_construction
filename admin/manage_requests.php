<?php
    ...
    if ($search_term !== '') {
        $where[] = "(name LIKE ? OR email LIKE ? OR tell LIKE ? OR subject LIKE ? OR message LIKE ?)";
        for ($i = 0; $i < 5; $i++) {
            $params[] = '%' . $search_term . '%';
        }
    }
    ...
    $total_sql = "SELECT COUNT(*) FROM requests $where_sql";
    $stmt = $pdo->prepare($total_sql);
    $stmt->execute($params);
    $total_requests = $stmt->fetchColumn();
    $total_pages = ceil($total_requests / $per_page);

    $sql = "SELECT * FROM requests $where_sql ORDER BY id DESC LIMIT $per_page OFFSET $offset";
...
<form action="manage_requests" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
    ...
    <div class="flex items-center space-x-4">
        ...
        <a href="manage_requests" class="text-red-500 text-base hover:underline">Clear Filters</a>
    </div>
</form>
    ...
    <table class="min-w-full leading-normal">
        <thead class="bg-gray-200">
            <tr>
                ...
                <th class="px-3 py-2 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Subject</th>
                <th class="px-3 py-2 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Message</th>
            ...
            <ul class="inline-flex items-center -space-x-px">
                <?php
                    ...
                    if ($current_page > 1) {
                        ...
                        echo '<li><a href="manage_requests?' . http_build_query($prev_page_params) . '" class="px-3 py-2 ml-0 leading-tight text-slate-500 bg-white border border-slate-300 rounded-l-lg hover:bg-slate-100 hover:text-slate-700">Previous</a></li>';
                    }
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        if ($i == 1 || $i == $total_pages || ($i >= $current_page - $range && $i <= $current_page + $range)) {
                            ...
                            if ($i == $current_page) {
                                ...
                                echo '<li><a href="manage_requests?' . http_build_query($page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 hover:bg-slate-100 hover:text-slate-700">' . $i . '</a></li>';
                            }
                        } else { if (!$show_ellipsis) { $show_ellipsis = true; } }
                    }
                    if ($current_page < $total_pages) {
                        $next_page_params = array_merge($pagination_params, ['page' => $current_page + 1]);
                        echo '<li><a href="manage_requests?' . http_build_query($next_page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 rounded-r-lg hover:bg-slate-100 hover:text-slate-700">Next</a></li>';
                    }
                ?>
            </ul>
        </nav>
            ...
            tableBody.addEventListener('click', function(event) {
                ...
                if (deleteButton) {
                    ...
                    if (confirm('Are you sure you want to permanently delete this request?')) {
                        ...
                        const tableRow = document.getElementById('request-row-' + requestId);

                        const formData = new FormData();
                        formData.append('id', requestId);

                        fetch('delete_request', { method: 'POST', body: formData })
