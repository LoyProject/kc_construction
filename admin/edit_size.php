<?php
    ...
    $size_id = filter_input(INPUT_GET, 'id', FILTER_VALIDATE_INT);
    $size = null;

    if (!$size_id) {
        $_SESSION['message'] = "Invalid size ID specified.";
        $_SESSION['message_type'] = "error";
        header("Location: manage_sizes");
        exit;
    }
    ...
    try {
        ...
        if (!$size) {
            $_SESSION['message'] = "Size not found.";
            $_SESSION['message_type'] = "error";
            header("Location: manage_sizes");
            exit;
        }
    } catch (PDOException $e) {
        error_log("Database error fetching size for edit: " . $e->getMessage());
        $_SESSION['message'] = "Error fetching size details.";
        $_SESSION['message_type'] = "error";
        header("Location: manage_sizes");
        exit;
    }

    $form_data = $_SESSION['form_data'] ?? $size;
    $form_errors = $_SESSION['form_errors'] ?? [];
    unset($_SESSION['form_data'], $_SESSION['form_errors']);
?>

<div class="md:col-span-1">
    <h1 class="text-xl font-bold mb-4 truncate">Edit size: <?php echo sanitize_output($size['name']); ?></h1>

    <form action="process_edit_size" method="POST">
        <input type="hidden" name="size_id" value="<?php echo sanitize_output($size['id']); ?>">

        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-slate-700 mb-1">Size Name <span class="text-red-500">*</span></label>
        ...
        <div class="flex items-center justify-end space-x-3 pt-4 border-t border-slate-200">
            ...
            <button type="submit" name="submit_edit_size" class="bg-brand-green hover:bg-green-700 text-white font-bold py-2 px-4 rounded-md shadow-sm">Save Changes</button>
        </div>
    </form>
</div>
