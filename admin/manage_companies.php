<?php
    ...
    $total_sql = "SELECT COUNT(*) FROM companies $where_sql";
    $stmt = $pdo->prepare($total_sql);
    $stmt->execute($params);
    $total_companies = $stmt->fetchColumn();
    $total_pages = ceil($total_companies / $per_page);

    $sql = "SELECT * FROM companies $where_sql ORDER BY id DESC LIMIT $per_page OFFSET $offset";
...
<div class="flex justify-between items-center mb-6">
    ...
    <a href="add_company" class="hidden inline-flex items-center justify-center py-2 px-4 border border-transparent shadow-sm text-sm font-medium rounded-md text-white bg-brand-blue hover:bg-blue-700">
...
<form action="manage_companies" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end mb-4">
    <div>
    ...
    <div class="flex items-center space-x-4">
        ...
        <a href="manage_companies" class="text-red-500 text-base hover:underline">Clear Filters</a>
    </div>
</form>
    ...
    <table class="min-w-full leading-normal">
        <thead class="bg-gray-200">
            <tr>
                ...
                <th class="px-3 py-2 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Email</th>
                <th class="px-3 py-2 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Phone</th>
                <th class="px-3 py-2 border-b-2 text-left text-xs font-semibold text-gray-600 uppercase">Schedule</th>
        ...
        <tbody>
            <?php
                if (empty($companies)) {
                    echo '<tr><td colspan="10" class="px-3 py-2 border-b text-center text-gray-500">No companies found.</td></tr>';
                } else { 
                    foreach ($companies as $company) {
                        echo '<tr id="company-row-' . htmlspecialchars($company['id'] ?? '') . '" class="text-xs text-left text-gray-700 hover:bg-gray-50 transition-colors">';
                            echo '<td class="px-3 py-2 border-b border-gray-200 font-semibold truncate"><img src="' . htmlspecialchars($company['logo'] ??'') . '" alt="" class="w-16 h-16 object-cover rounded"></td>';
                            echo '<td class="px-3 py-2 border-b border-gray-200 max-w-sm truncate" title="' . htmlspecialchars($company['name'] ?? '') . '">' . htmlspecialchars($company['name'] ?? '') . '</td>';
                            echo '<td class="px-3 py-2 border-b border-gray-200 max-w-sm truncate" title="' . htmlspecialchars($company['email'] ?? '') . '">' . htmlspecialchars($company['email'] ?? '') . '</td>';
                            echo '<td class="px-3 py-2 border-b border-gray-200 max-w-sm truncate" title="' . htmlspecialchars($company['tell'] ?? '') . '">' . htmlspecialchars($company['tell'] ?? '') . '</td>';
                            echo '<td class="px-3 py-2 border-b border-gray-200 max-w-sm truncate" title="' . htmlspecialchars($company['schedule'] ?? '') . '">' . htmlspecialchars($company['schedule'] ?? '') . '</td>';
                            echo '<td class="px-3 py-2 border-b border-gray-200 max-w-sm truncate" title="' . htmlspecialchars($company['created_at'] ?? '') . '">' . htmlspecialchars($company['created_at'] ?? '') . '</td>';
                            echo '<td class="w-24 px-3 py-2 border-b border-gray-200 space-x-3 truncate">';
                                if (isset($_SESSION['role']) && $_SESSION['role'] !== 'View') {
                                    echo '<a href="edit_company?id=' . urlencode($company['id']) . '" class="text-indigo-600 hover:text-indigo-900">
            ...
            <ul class="inline-flex items-center -space-x-px">
                <?php
                    ...
                    if ($current_page > 1) {
                        ...
                        echo '<li><a href="manage_companies?' . http_build_query($prev_page_params) . '" class="px-3 py-2 ml-0 leading-tight text-slate-500 bg-white border border-slate-300 rounded-l-lg hover:bg-slate-100 hover:text-slate-700">Previous</a></li>';
                    }
                    ...
                    for ($i = 1; $i <= $total_pages; $i++) {
                        if ($i == 1 || $i == $total_pages || ($i >= $current_page - $range && $i <= $current_page + $range)) {
                            ...
                            if ($i == $current_page) {
                                ...
                                echo '<li><a href="manage_companies?' . http_build_query($page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 hover:bg-slate-100 hover:text-slate-700">' . $i . '</a></li>';
                            }
                        } else { if (!$show_ellipsis) { $show_ellipsis = true; } }
                    }
                    if ($current_page < $total_pages) {
                        $next_page_params = array_merge($pagination_params, ['page' => $current_page + 1]);
                        echo '<li><a href="manage_companies?' . http_build_query($next_page_params) . '" class="px-3 py-2 leading-tight text-slate-500 bg-white border border-slate-300 rounded-r-lg hover:bg-slate-100 hover:text-slate-700">Next</a></li>';
                    }
                ?>
            </ul>
        </nav>
            ...
            tableBody.addEventListener('click', function(event) {
                ...
                if (deleteButton) {
                    ...
                    if (confirm('Are you sure you want to permanently delete this company?')) {
                        ...
                        const tableRow = document.getElementById('company-row-' + companyId);

                        const formData = new FormData();
                        formData.append('id', companyId);

                        fetch('delete_company', { method: 'POST', body: formData })
